...
Options:
  --purpose "<text>"      Project purpose for docs/arch.md
  --stack "<text>"        Stack summary for docs/arch.md
  --boundaries "<text>"   Scope boundaries for docs/arch.md
...
if [[ "$DRY_RUN" -eq 1 ]]; then
  ...
  echo "[dry-run] append initial log entry to: $DEST/log.md"
  echo "[dry-run] write initial architecture facts to: $DEST/docs/arch.md"
  exit 0
fi
